...
const SaveCancelControl = ({ cancelClick, saveClick, itemPath, saveEnabled }) => {
    return (
        <div className="flex space-x-1">
            <LinkButton href={itemPath} navigationClick={cancelClick}>Cancel</LinkButton>
            <Button onClick={saveClick} enabled={saveEnabled} type={ saveEnabled ? "primary" : "info" }>Save</Button>
        </div>)
}
...
const SavedErrors = ({ errors = [] }) => {
    return (
        <div>
            <p className="text-rose-600 text-lg">Errors from Save:</p>
            <ul className="list-disc">
                {errors.map((error) =>
                    <li key={error.description} className="text-rose-600 text-base">
                        {error.description}
                    </li>
                )}
            </ul>
...
const EditPage = ({ item, path }) => {
    ...
    const editable = (session, item) => {
        ...
        const loggedIn = session != null && ("auth.userid" in session) && session["auth.userid"].length > 0
                ...
                const errors = response.errors.map( (err) => (
                    {
                        ...
                        keys: err.names.map((val) => {(`\`${val}\``)}).join(", "),
                    }))
                setErrors(errors)
            }
        })
    ...
    return (
        <div className="space-y-1">
            <EditJson text={text} onChange={onChange} enabled={editorStatus.canEdit} errors={editorStatus.errors}/>
            <div style={{
                position: "relative",
                boder: "1px solid lightgray",
            }}>
                <SaveCancelControl cancelClick={() => ({})} saveClick={save} itemPath={path} saveEnabled={editorStatus.canSave}/>
                { saveErrors.length > 0 && (
                    <SavedErrors errors={saveErrors}/>
                )}
            </div>
        </div>
    )
