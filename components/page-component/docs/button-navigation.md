...
Fourth Topic Title=/topic/fourth|[fourth-topic-1,fourth-topic-2]|The **fourth** topic’s markdown.
#first-topic=<svg>…</svg>
#second-topic=<svg>…</svg>
#third-topic=<svg>…</svg>
#fourth-topic-1=<svg>…</svg>
#fourth-topic-2=<svg>…</svg>
```
...
- **icon SVG ID** — To make these button definition lines manageable, SVG for the icons gets defined on their own lines. You give the id of the SVG that you provide later in this edit block. Generally use a shishkebab-case string for this ID, though really any string would work as long as it doesn’t contain the pipe character which the page-component mechanism uses to separate elements on the right side of the equals sign.
  ...
  - Multiple Icons — In most cases you define a single icon on a button navigation card. But you can also define multiple icons on the card sitting next to each other. To do this, specify the icon IDs within square brackets, and separate each icon ID within those brackets by a comma with no spaces. See an example above for the “Fourth Topic Title” which includes the `fourth-topic-1` and `fourth-topic-2` icons.
