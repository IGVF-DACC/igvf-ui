 ...
 * the filter. It uses a logarithmic scale used so that the user can select a finer range of values
 ...
 * while variables with names including "sliderMin" or "sliderMax" are the slider values.
...
export default function FileSizeTerms({
  ...
  if (facet.type !== "stats") {
    console.error(
      `FileSizeTerms: Expected a stats facet but got a ${facet.type} facet. Check the search config for this facet.`
    );
    return null;
  }
