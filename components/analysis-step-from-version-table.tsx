...
// root
import { SORT_FALLBACK_VALUE } from "../lib/constants";
...
const analysisStepVersionColumns: SortableGridConfig<
...
>[] = [
  ...
  },
  {
    ...
    display: ({ source }) =>
      typeof source.analysis_step === "object"
        ? source.analysis_step.title
        : "",
    sorter: (item) =>
      typeof item.analysis_step === "object"
        ? item.analysis_step.title.toLowerCase()
        : SORT_FALLBACK_VALUE,
  },
  {
    ...
    display: ({ source }) => {
      const types =
        typeof source.analysis_step === "object"
          ? source.analysis_step.analysis_step_types
          : [];
  ...
  },
  {
    ...
    display: ({ source }) => {
      const inputTypes =
        typeof source.analysis_step === "object"
          ? source.analysis_step.input_content_types
          : [];
  ...
  },
  {
    ...
    display: ({ source }) => {
      const outputTypes =
        typeof source.analysis_step === "object"
          ? source.analysis_step.output_content_types
          : [];
 ...
 */
export function AnalysisStepFromVersionTable({
