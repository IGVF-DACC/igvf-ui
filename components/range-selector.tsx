 ...
 * @param splitValue - The average value of the min and max range inputs
 */
function update({ container, min, max }: RangeSelectorProps): {
  ...
  const splitValue = Math.round(average);
  ...
  // the split value.
  const domSplitValue = splitValue.toString();
  ...
  const minWidth = Math.floor(
    THUMB_SIZE + ((splitValue - rangeMin) / range) * realWidth
  );
  const maxWidth = Math.floor(
    THUMB_SIZE + ((rangeMax - splitValue) / range) * realWidth
  );
