...
export function SecDir({ sections }: { sections: SectionList }) {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <MenuButton className="flex justify-center rounded-sm bg-menu-trigger p-1 data-[hover]:bg-menu-trigger-hover data-[open]:bg-menu-trigger-open">
        <Icon.SectionDirectory className="h-5 w-5" />
      </MenuButton>
      <MenuItems
        transition
        anchor="bottom end"
        className="z-20 rounded-lg border border-menu-items bg-menu-items shadow-lg"
      >
          ...
          <MenuItem key={section.id}>
            {({ close }) => (
              <a
                ...
                className="block px-4 py-0.5 text-sm font-medium text-menu-item no-underline hover:bg-menu-item-hover hover:text-menu-item-hover"
              >
                {ItemRenderer ? (
                  <ItemRenderer
                    section={section}
