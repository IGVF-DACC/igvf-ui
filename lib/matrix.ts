...
/**
 * Matrix No-Data Tuples
 *
 * The backend matrix mechanism has an undocumented feature that allows items without an element of
 * the matrix axes to appear as "no-data" results. The no-data tuples represent cases where a
 * particular combination of axis values has no associated data. Here's an example:
 *
 * 'y': {
 *   'group_by': [
 *     'lab.title',
 ...
 *  },
 *
 * Using this configuration with the `/matrix` endpoint causes a crash or incorrect results. You must
 * use the `/missing-matrix` endpoint to process this tuple.
 *
 * If, instead of this tuple, you simply used `'assay_titles'` the `/matrix` endpoint returns results
 * that include an `assay_titles` property. Objects missing the `assay_titles` property are excluded
 * from the results.
 *
 * With the tuple, and using the `/missing-matrix` endpoint, the returned results include objects
 * that are missing the `assay_titles` property, represented by the `'no_assay_titles'` value in the
 * tuple, as if it were any other value in the axis.
 */
...
/**
 * Type guard to check if a value has the form of a MatrixBucketWrapper object.
 *
 ...
 * @param component - React component to use for rendering each empty cell
 * @returns Array of cells that would render as empty
 */
export function generateEmptyRowCells(
  length: number,
