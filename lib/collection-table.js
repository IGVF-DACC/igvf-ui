 ...
 * @returns {string} TSV data for the given collection
 */
export const generateTsvFromCollection = (collection, columns) => {
  ...
  const tsvContent = `data:text/tab-separated-values,${columnTitlesAndRows
    .map((row) => row.join("\t"))
    .join("\n")}`;
  return encodeURI(tsvContent);
};
