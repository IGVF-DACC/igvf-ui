 ...
 * @param {object} searchResults Search results from igvfd
 * @param {object} collectionTitles Map of collection identifiers to titles
 * @returns {string} Page title for search results page
 */
export function composeSearchResultsPageTitle(searchResults, collectionTitles) {
  if (collectionTitles && searchResults["@graph"].length > 0) {
    const types = getSearchResultItemTypes(searchResults);
    const title = collectionTitles[types[0]];
    if (title) {
