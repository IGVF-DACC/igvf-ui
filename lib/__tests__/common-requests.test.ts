...
describe("Test all the common requests", () => {
  ...
  test("requestFiles function", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/search-quick/?type=File&field=@type&field=accession&field=aliases&field=collections&field=content_summary&field=content_type&field=controlled_access&field=creation_timestamp&field=derived_from&field=derived_manually&field=external_host_url&field=externally_hosted&field=file_format&field=file_size&field=file_set&field=filtered&field=flowcell_id&field=href&field=illumina_read_type&field=imaging_platform&field=index&field=input_file_for&field=lab.@id&field=lab.title&field=lane&field=quality_metrics&field=read_names&field=reference_files&field=seqspecs&field=seqspec_document&field=sequencing_platform&field=sequencing_run&field=submitted_file_name&field=status&field=summary&field=tile&field=workflows&field=upload_status&@id=/sequence-files/IGVFFI4067OVRO/&@id=/sequence-files/IGVFFI1165AJSO/&limit=2",
      expect.anything()
    );
    expect(result).toHaveLength(2);
    expect(result[0]).toEqual(mockResult["@graph"][0]);
