...
describe("QueryString Class", () => {
  ...
  it("handles a query string with a leading ? the same as without", () => {
    let query = new QueryString("?type=Experiment");
    expect(query.format()).toEqual("type=Experiment");

    query = new QueryString("type=Experiment");
    expect(query.format()).toEqual("type=Experiment");
  });
