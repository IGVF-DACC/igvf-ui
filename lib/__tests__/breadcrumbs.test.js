...
import { generateParentPaths, REPLACE_TYPE } from "../breadcrumbs";
...
describe("Test breadcrumb composition and rendering functionality", () => {
  ...
  it("builds an item breadcrumb data properly", async () => {
    ...
    let breadcrumbs = await buildBreadcrumbs(labItemData, "title");
    ...
    breadcrumbs = await buildBreadcrumbs(unknownItemData, "title");
    expect(breadcrumbs).toHaveLength(2);
  ...
  it("builds an item breadcrumb when profile data isn't available", async () => {
    ...
    const breadcrumbs = await buildBreadcrumbs(humanDonorItemData, "accession");
    expect(breadcrumbs).toHaveLength(2);
