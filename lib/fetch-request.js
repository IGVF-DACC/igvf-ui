...
export default class FetchRequest {
   ...
   * error detected by the server, or a network error.
   ...
   */
  get #baseUrl() {
    return this.#isServer ? SERVER_URL : API_URL;
  }
   ...
   */
  async getObject(path, defaultErrorValue) {
   ...
   */
  async getMultipleObjects(paths, defaultErrorValue) {
   ...
   * requested objects that fail to get retrieved don't get embedded, leaving the path unchanged.
   * If you need to embed multiple properties, call this method once for each property.
  ...
  async getCollection(collection, defaultErrorValue) {
    return this.getObject(`/${collection}/`, defaultErrorValue);
  }
   ...
   * @returns Reponse from PATCH request
   */
